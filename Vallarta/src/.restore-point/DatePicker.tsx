...
export default function DatePicker({
  ...
  const handleDayClick = (dayVal: number) => {
    const clickedDateStr = toDateString(currentYear, currentMonth, dayVal);
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute z-50 left-0 right-0 mt-2 p-1 rounded-lg cal-date-picker-outer"
        >
              ...
              {days.map((d, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      relative h-9 rounded flex flex-col items-center justify-center transition-all duration-150 cursor-pointer cal-picker-day
                      ${isCheckIn || isCheckOut ? 'cal-picker-day--selected' : ''}
                      ${inRange && !isCheckIn && !isCheckOut ? 'cal-picker-day--range' : ''}
                      ${!isCheckIn && !isCheckOut && !inRange ? 'cal-picker-day--hover' : ''}
                    `}
                  >
